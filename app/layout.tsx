...
const themeScript = `(function(){try{var r=document.documentElement;var t=localStorage.getItem('theme');if(t!=='light'&&t!=='dark'){t=matchMedia('(prefers-color-scheme: dark)').matches?'dark':'light';}r.dataset.theme=t;r.style.colorScheme=t;var acc=localStorage.getItem('accent')||'indigo';r.dataset.accent=acc;if(acc==='custom'){var ac=localStorage.getItem('accentCustom'),ah=localStorage.getItem('accentCustomH');if(ac){r.style.setProperty('--accent',ac);r.style.setProperty('--accent-hover',ah||ac);}}r.dataset.logo=localStorage.getItem('logo')||'mark';r.dataset.bg=localStorage.getItem('bg')||'none';r.dataset.font=localStorage.getItem('font')||'inter';r.dataset.size=localStorage.getItem('size')||'md';r.dataset.surface=localStorage.getItem('surface')||'solid';r.dataset.tilefx=localStorage.getItem('tilefx')||'none';r.dataset.sound=localStorage.getItem('sound')||'sfx';r.dataset.cursor=localStorage.getItem('cursor')||'system';r.dataset.cursorfx=localStorage.getItem('cursorfx')||'none';var tc=localStorage.getItem('tzChoice');var tz=(tc&&tc!=='auto')?tc:(Intl.DateTimeFormat().resolvedOptions().timeZone||'UTC');if(!/(?:^|; )tz=/.test(document.cookie)){document.cookie='tz='+tz+'; path=/; max-age=31536000; samesite=lax';}}catch(e){}})();`;
